...
from tornado.web import HTTPError, asynchronous, RequestHandler
...
class QueryHandler(APIHandler):
    @gen.coroutine
    def get(self):
        cursor = self.database.post.find()
        json_data = []
        for t in (yield cursor.to_list(length=30)):
            j = {
                '_id': text_type(t['_id']),
                'title': t['title'],
...
class WriteHandler(APIHandler):
    @gen.coroutine
    def post(self):
        yield self.database.post.insert({'title': self.post_data['title'], 'created': datetime.now()})
        self.success()
...
class Application(tornado.web.Application):
    def __init__(self):
        ...
        # 创建一个数据库连接池
        self.db_client = motor.MotorClient(MONGO_HOST, MONGO_PORT, max_pool_size=200)
        # 验证数据库用户名和密码
