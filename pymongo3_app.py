...
class PostBaseDataHandler(APIHandler):
    def post(self):
        times = self.post_data['times']
        per_time = self.post_data['per_time']

        for t in xrange(times):
            self.database.post.insert_many(
                [{'title': 'Post Title', 'created': datetime.now()} for t in xrange(per_time)]
            )
        self.success(data={'count': self.database.post.count()})


class DeleteManyHandler(APIHandler):
    def post(self):
        dt = datetime.strptime(self.post_data['datetime'], '%Y-%m-%d %H:%M:%S')
        self.database.post.delete_many({'created': {'$lte': dt}})
        self.success(data={'count': self.database.post.count()})
...
class Application(tornado.web.Application):
    def __init__(self):
        ...
        handlers = [
            ...
            (r'/api/posts/delete_all/?', DeleteManyHandler),
